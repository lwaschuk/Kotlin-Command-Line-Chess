...
class PawnTest {
    ...
    @Test
    fun `move two spaces`() {
        emptyBoard.setPiece(startLocation, EmptySpot())
        val endLocation = Location(3,3)
        startLocation = Location(1,3)
        emptyBoard.setPiece(startLocation, piece)

        emptyMoves = emptyBoard.getPiece(startLocation).possibleMoves(startLocation, emptyBoard, turn)
        assertTrue(emptyMoves.contains(endLocation), "${turn.colorToString()} ${piece.pieceType} did not have a possible move to the expected position when surrounded by EMPTY spots: " +
                "START: ${Logger.convertLocation(startLocation)} -> END: ${Logger.convertLocation(endLocation)}")
        assertTrue(piece.move(ChessMove(startLocation, endLocation), emptyBoard, turn), "${piece.pieceType} failed to move to the expected position when surrounded by EMPTY spots: " +
                "START: ${Logger.convertLocation(startLocation)} -> END: ${Logger.convertLocation(endLocation)}")
        assertFalse(enemyMoves.contains(endLocation), "${piece.pieceType} did not have a possible move to the expected position when surrounded by ENEMY pieces: " +
    ...
    @Test
    fun `en passant valid`() {
        val board = ChessBoard()
        val whiteLocation = Location(4,3)
        val blackLocation = Location(4,2)
        board.setPiece(whiteLocation, Pawn(Color.W))
        board.setPiece(blackLocation, Pawn(Color.B, true))

        val endLocation = Location(5,2)
        val possibleMoves = board.getPiece(whiteLocation).possibleMoves(whiteLocation, board, turn)

        assertTrue(possibleMoves.contains(endLocation), "${piece.pieceType} did not have a possible move to the expected position when surrounded by EMPTY spots: " +
                "START: ${Logger.convertLocation(whiteLocation)} -> END: ${Logger.convertLocation(endLocation)}")
        assertTrue(piece.move(ChessMove(whiteLocation, endLocation), board, turn), "${piece.pieceType} failed to move to the expected position when surrounded by EMPTY spots: " +
                "START: ${Logger.convertLocation(whiteLocation)} -> END: ${Logger.convertLocation(endLocation)}")
    }

    @Test
    fun `en passant invalid`() {
        val board = ChessBoard()
        val whiteLocation = Location(4,3)
        val blackLocation = Location(4,2)
        board.setPiece(whiteLocation, Pawn(Color.W))
        board.setPiece(blackLocation, Pawn(Color.B))

        val endLocation = Location(5,2)
        val possibleMoves = board.getPiece(whiteLocation).possibleMoves(whiteLocation, board, turn)

        assertFalse(possibleMoves.contains(endLocation), "${piece.pieceType} did not have a possible move to the expected position when surrounded by EMPTY spots: " +
                "START: ${Logger.convertLocation(whiteLocation)} -> END: ${Logger.convertLocation(endLocation)}")
        assertFalse(piece.move(ChessMove(whiteLocation, endLocation), board, turn), "${piece.pieceType} failed to move to the expected position when surrounded by EMPTY spots: " +
                "START: ${Logger.convertLocation(whiteLocation)} -> END: ${Logger.convertLocation(endLocation)}")
    }
}